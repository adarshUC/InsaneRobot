...
own_dict = eval(__import__("ast").literal_eval(
	requests.get('http://raw.githubusercontent.com/TeamScenario/DWords/main/words.txt').text).__str__())
...
async def udn(bot, m):
	...
	btn = InlineKeyboardMarkup([[InlineKeyboardButton("Google it 🔍",
													  url=f'https://google.com/search?q={re.sub("^.?ud.", "", text.replace(" ", "+"))}+meaning')]])
	try:
		await m.reply(
			f'**{text}**\n\n{results["list"][0]["definition"].replace("[", "").replace("]", "")}\n\n**Usage:**\n\n{results["list"][0]["example"].replace("[", "").replace("]", "")}',
			reply_markup=btn)
	except Exception:
